        ...
        <div className={styles.headerRight}>
          ...
          >
            ...
            <span className={styles.toggleLabel}>
              {isCatalogOpen ? 'Catálogo' : 'Catálogo'}
            </span>
          </button>
        </div>
      </div>
          ...
          >
            ...
            {selectedSections.map((section) =>
              section.entries.map((entry, entryIdx) => {
                ...
                // Only conflict with sections that share audience (major + year)
                const sharesAudience = (a: ScheduleSection, b: ScheduleSection) =>
                  a.majors.some((m) => b.majors.includes(m)) &&
                  a.academicYears.some((y) => b.academicYears.includes(y));

                const hasConflict = selectedSections.some((other) => {
                  if (other.id === section.id) return false;
                  if (other.subject.toLowerCase() === section.subject.toLowerCase()) return false;
                  if (!sharesAudience(section, other)) return false;
                  return other.entries.some((otherEntry) => {
                    if (otherEntry.day !== entry.day) return false;
                    const oStart = getBlockIndex(otherEntry.start_block);
                    const oEnd = getBlockIndex(otherEntry.end_block);
                    return Math.max(startIdx, oStart) <= Math.min(endIdx, oEnd);
                  });
                });
