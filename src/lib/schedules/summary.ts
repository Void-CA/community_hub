...
export function getScheduleSummary(entries: ScheduleEntry[]) {
  ...
  const subjectCounts = entries.reduce(
    (acc, entry) => {
  ...
  const mostPopular = Object.entries(subjectCounts).sort(
    ([, a], [, b]) => b - a,
  )[0];
  ...
  return {
    ...
    mostPopularSubject: mostPopular
      ? { name: mostPopular[0], count: mostPopular[1] }
      : null,
  };
}
